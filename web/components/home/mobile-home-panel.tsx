...
function SearchIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
      className="mobile-home__icon-svg"
    >
      <circle
        cx="11"
        cy="11"
...
function TopicIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
      className="mobile-home__row-icon"
    >
      <path
...
function shuffleTopics(items: TopicItem[]) {
  ...
  for (let index = nextTopics.length - 1; index > 0; index -= 1) {
    ...
    [nextTopics[index], nextTopics[randomIndex]] = [
      nextTopics[randomIndex],
      nextTopics[index],
    ];
  }
  return nextTopics;
...
export default function MobileHomePanel({
  ...
  const topicsSignature = useMemo(
    () =>
      topics
        .map(
          (topic) => `${topic.label}|${topic.prompt}|${topic.chipLabel ?? ""}`,
        )
        .join("::"),
    [topics],
  );
    ...
    <section className="mobile-home" aria-labelledby={introId}>
      {showSearch ? (
        <>
          <div className="mobile-home__search-lead">
            <p className="mobile-home__search-eyebrow">
              Search Army Regs with AI
            </p>
          </div>
          ...
          >
            ...
            <div className="mobile-home__search-input-wrap">
              {!value ? (
                <span
                  className="mobile-home__search-placeholder"
                  aria-hidden="true"
                >
                  Ask your question...
                </span>
      ...
      {mode === "landing" ? (
        <article className="mobile-home__intro-card">
          ...
          <p className="mobile-home__intro-body">Ask plain-language questions.</p>
          <p className="mobile-home__intro-body mobile-home__intro-body--secondary">
            Get AI-generated answers tied to exact regulation paragraphs, then
            open the source PDF to inspect the text in context.
          </p>
          <button
            type="button"
          ...
          {isLearnMoreOpen ? (
            <div className="mobile-home__learn-more-content" id={learnMoreId}>
              <p className="mobile-home__learn-more-block">
                <strong>Built for research:</strong> ArmyRegs.ai helps Soldiers,
                leaders, and legal professionals find and understand Army
                regulations faster.
              </p>
              <p className="mobile-home__learn-more-block">
                <strong>Citation-backed answers:</strong> each response includes
                source citations so you can trace the answer back to the
                underlying authority.
              </p>
              <p className="mobile-home__learn-more-block">
                <strong>Open the source:</strong> click a citation to view the
                regulation PDF and review the quoted passage in context.
              </p>
              <p className="mobile-home__learn-more-block">
                <strong>Save your work:</strong>{" "}
                <Link href="/signup">Create an account</Link> to keep research
        ...
        <>
          ...
          {regulationSyncLabel ? (
            <aside
              ...
              aria-label="Regulation sync information"
            >
              <p className="mobile-home__sync-label">Last Regulation sync</p>
              <p className="mobile-home__sync-value">{regulationSyncLabel}</p>
            </aside>
          ) : null}

          <aside
            className="chat-empty-disclaimer mobile-home__notice-card"
            aria-label="Usage notice"
          >
            <h2 className="chat-empty-disclaimer__title">Notice</h2>
            <p className="chat-empty-disclaimer__text">
              Do not include any Personally Identifying Information (PII), HIPAA
              Protected Health Information (PHI), or classified (including CUI)
              information.
            </p>
            <p className="chat-empty-disclaimer__text">
              This tool does not constitute legal advice and should not be used
              as a substitute for consulting the actual regulations or a legal
              professional. Always verify the information provided by this tool
              against the official Army Regulations and consult with a qualified
              legal advisor for any specific questions or concerns.
            </p>
          </aside>
        </>
      ) : null}
